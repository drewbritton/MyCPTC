...
namespace CPTCAppNew {
    ...
    public class SchoolEventsRepository : ISchoolEventsRepository {
        ...
        public const string apiUrl = "https://cptc.azurewebsites.net";
   ...
   //     }
		...
		public async Task<List<SchoolEvents>> GetEventsAsync() {
			...
			if (result.IsSuccessStatusCode && result.StatusCode == System.Net.HttpStatusCode.OK) {
				...
				List<SchoolEvents> status = JsonConvert.DeserializeObject<List<SchoolEvents>>(json);
				return (List<SchoolEvents>) status;
			}
		...
		public async Task<List<SchoolEvents>> TopEventsAsync() {
			...
			if (result.IsSuccessStatusCode && result.StatusCode == System.Net.HttpStatusCode.OK) {
				...
				foreach (var i in status) {
					...
					count++;
					if (count == 3)
						return (List<SchoolEvents>) topEvents;
				}
